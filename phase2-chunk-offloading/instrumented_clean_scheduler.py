...
class OffloadingConnectorScheduler:
    ...
    def __init__(self, spec: OffloadingSpec):
        ...
        # CHUNKPROBE: dump the offload geometry so we can see factor>1 grouping.
        logger.info(
            "CHUNKPROBE CONFIG num_workers=%d block_size_factor=%d num_groups=%d",
        ...
        for gc in self.config.kv_group_configs:
            logger.info(
                "CHUNKPROBE CONFIG group_idx=%d gpu_block_size=%d "
                "offloaded_block_size=%d hash_block_size_factor=%d "
                "sliding_window_size_in_blocks=%s",
                gc.group_idx,
                gc.gpu_block_size,
                gc.offloaded_block_size,
                gc.hash_block_size_factor,
                gc.sliding_window_size_in_blocks,
            )
    ...
    def update_state_after_alloc(
        ...
        # CHUNKPROBE: non-empty keys_to_load means vLLM is REUSING offloaded
        # chunks by reloading them CPU->GPU (the prefix was evicted from GPU
        # but recovered from the CPU tier). This proves chunk reuse works.
        if keys_to_load:
            logger.info(
                "CHUNKPROBE REUSE/LOAD req=%s num_chunks_loaded=%d "
                "(~%d tokens) chunk_key_hashes=%s",
                request.request_id,
                len(keys_to_load),
                num_external_tokens,
                [get_offload_block_hash(k).hex() for k in keys_to_load],
            )
    ...
    def _build_store_jobs(
        ...
        for req_id, new_block_id_groups, preempted in yield_req_data(scheduler_output):
            ...
            for group_config, group_state in zip(
                ...
                for i, (offload_key, block_id) in enumerate(
                    zip(offload_keys, offload_block_ids)
                ):
                    if block_id != 0:
                        new_offload_keys.append(offload_key)
                        # CHUNKPROBE: one OffloadKey == one CPU chunk covering
                        # block_size_factor GPU blocks / hash_block_size_factor
                        # request hash-blocks. Show the chunk hash vs the
                        # constituent per-block hashes it was derived from.
                        chunk_idx = start_block_idx + i
                        key_hash = get_offload_block_hash(offload_key).hex()
                        constituent = [
                            (h.hex() if h is not None else None)
                            for h in req.block_hashes[
                                hbf * chunk_idx : hbf * (chunk_idx + 1)
                            ]
                        ]
                        logger.info(
                            "CHUNKPROBE STORE req=%s group=%d chunk_idx=%d "
                            "offloaded_block_size=%d num_gpu_blocks_in_chunk=%d "
                            "key_hash=%s constituent_block_hashes=%s "
                            "chunk_hash_eq_last_block=%s",
                            req_id,
                            ...
                            key_hash,
                            constituent,
                            bool(constituent) and key_hash == constituent[-1],
                        )
    ...
    def take_events(self) -> Iterable[KVCacheEvent]:
        ...
        for event in self.manager.take_events():
            ...
            # CHUNKPROBE: this is exactly the payload a downstream router (e.g.
            # Dynamo / llm-d) receives. Note token_ids=[] and block_size=0 for
            # stored events: the per-chunk content is NOT self-describing.
            logger.info(
                "CHUNKPROBE EVENT type=%s medium=%s num_hashes=%d "
                "hashes=%s token_ids_len=%d block_size=%d parent=None",
                "removed" if event.removed else "stored",
                event.medium,
                len(block_hashes),
                [h.hex() for h in block_hashes],
                0,
                0,
            )
            if event.removed:
