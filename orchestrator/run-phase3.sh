#!/usr/bin/env bash
# Phase 3 sweep: pl-finer + longer-prefix + lmbench-qps-sweep
set +e; set +u
...
run_cycle() {
  ...
  collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "$workload_json" "$c" "$rr" "OK_PHASE3"
}
...
run_lmbench() {
  ...
  log ""
  log "============================================================"
  log "PHASE3-LMBENCH $cycle_id  | $label  QPS=$qps"
  log "============================================================"
  cold_restart "$image" "$h" "$disk" "$pl"
  ...
  wait
  collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "lmbench" "64" "$qps" "OK_PHASE3"
}

# stage prefix configs (30K and 40K) — generated on-the-fly into bench pod
kubectl exec $CLIENT -n $NS -- bash -c '
python3 << "PY"
import json
src = json.load(open("/work/multi_turn/generate_multi_turn_longbench.json"))
for prefix, n_conv in [(30000, 64), (40000, 60)]:
    c = json.loads(json.dumps(src))
    c["prompt_input"]["prefix_num_tokens"]["value"] = prefix
    c["num_conversations"] = n_conv
    fn = f"/work/multi_turn/cfg_prefix_{prefix}.json"
    json.dump(c, open(fn, "w"))
    print(f"staged {fn}: conv_prefix={prefix}, num_conv={n_conv}")
PY
'

log "=========================="
log "PHASE 3 START $(date)"
log "=========================="

LONG=generate_multi_turn_longbench.json

# ---------- PHASE A: pl-finer sweep (3 cycles, ~30 min) ----------
log ""; log "##### PHASE A: pl-finer sweep #####"
for pl in 5 25 50; do
  run_cycle "ph3-a-pl${pl}"  "tunable h=1.0 pl=$pl"  $TUNABLE_IMG "1.0" "0.25" "$pl"  $LONG 64 0.5
done

# ---------- PHASE B: longer prefix (4 cycles, ~45 min) ----------
log ""; log "##### PHASE B: longer conv_prefix #####"
for prefix in 30000 40000; do
  run_cycle "ph3-b-baseline-prefix${prefix}"  "baseline prefix=${prefix}"  $BASELINE_IMG ""    ""     ""    "cfg_prefix_${prefix}.json" 64 0.5
  run_cycle "ph3-b-best-prefix${prefix}"      "best prefix=${prefix}"      $TUNABLE_IMG  "1.0" "0.25" "100" "cfg_prefix_${prefix}.json" 64 0.5
done

# ---------- PHASE C: LMBench QPS sweep (12 cycles, ~2h) ----------
log ""; log "##### PHASE C: LMBench QPS sweep #####"
for qps in 1 4 8 16 32 64; do
  run_lmbench "ph3-c-baseline-qps${qps}"  "lmbench baseline QPS=$qps"  $BASELINE_IMG ""    ""    $qps
  run_lmbench "ph3-c-best-qps${qps}"      "lmbench best QPS=$qps"      $TUNABLE_IMG  "1.0" "100" $qps
done
