#!/usr/bin/env bash
# Phase 4: Fill in 2 control lines (baseline+pl100, treatment-default) for each sweep figure
#  - T2 concurrency: 7 c values × 2 new lines = 14 cycles (skip c=192, budget-limited)
#  - T4 prefix: 3 sizes × 2 new lines = 6 cycles (skip 15K, already have all 4 from T1)
#  - T6 rr: 6 rr values × 2 new lines = 12 cycles (skip rr=0.5, already in T1)
#  - LMBench QPS: 6 QPS × 2 new lines = 12 cycles
# Total: 44 cycles, est ~7-8 hours
set +e; set +u
...
run_cycle() {
  ...
  collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "$workload_json" "$c" "$rr" "OK_PHASE4"
}
...
run_lmbench() {
  ...
  wait
  collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "lmbench" "64" "$qps" "OK_PHASE4"
}

log "=========================="
log "PHASE 4 START $(date)"
log "=========================="

LONG=generate_multi_turn_longbench.json

# ---------- T2: concurrency controls — 14 cycles ----------
log ""; log "##### T2: concurrency controls (baseline+pl100, treatment-default) #####"
# T2 already has baseline-default and best for c=8/16/32/64/96/128
# Skip c=192 (text budget). Add 2 control lines for the remaining 6 c values.
for c in 8 16 32 64 96 128; do
  run_cycle "ph4-t2-baseline-pl100-c${c}"  "baseline+pl100 c=$c"  $BASELINE_IMG ""    ""     "100" $LONG $c 0.5
  run_cycle "ph4-t2-treatment-default-c${c}"  "treatment-default c=$c"  $TUNABLE_IMG  "0.75" "0.25" "1"   $LONG $c 0.5
done

# ---------- T4: conv_prefix controls — 6 cycles ----------
log ""; log "##### T4: conv_prefix controls #####"
# T4 has baseline-default + best for 5K, 25K, 30K. (15K already has all 4 lines from T1.)
# Add 2 control lines for 5K, 25K, 30K.
for prefix in 5000 25000 30000; do
  cfg="cfg_prefix_${prefix}.json"
  run_cycle "ph4-t4-baseline-pl100-prefix${prefix}"  "baseline+pl100 prefix=${prefix}"  $BASELINE_IMG ""    ""     "100" $cfg 64 0.5
  run_cycle "ph4-t4-treatment-default-prefix${prefix}"  "treatment-default prefix=${prefix}"  $TUNABLE_IMG  "0.75" "0.25" "1"   $cfg 64 0.5
done

# ---------- T6: rr controls — 12 cycles ----------
log ""; log "##### T6: rr controls #####"
# T6 already has baseline-default + best for rr=0.1,0.25,0.5,0.75,1.0,2.0,4.0 (7 values)
# rr=0.5 already has all 4 lines from T1. Add 2 control lines for the other 6 rr values.
for rr in 0.1 0.25 0.75 1.0 2.0 4.0; do
  run_cycle "ph4-t6-baseline-pl100-rr${rr}"  "baseline+pl100 rr=$rr"  $BASELINE_IMG ""    ""     "100" $LONG 64 $rr
  run_cycle "ph4-t6-treatment-default-rr${rr}"  "treatment-default rr=$rr"  $TUNABLE_IMG  "0.75" "0.25" "1"   $LONG 64 $rr
done

# ---------- LMBench QPS controls — 12 cycles ----------
log ""; log "##### LMBench QPS controls #####"
for qps in 1 4 8 16 32 64; do
  run_lmbench "ph4-lm-baseline-pl100-qps${qps}"  "lmbench baseline+pl100 QPS=$qps"  $BASELINE_IMG ""     "100" $qps
  run_lmbench "ph4-lm-treatment-default-qps${qps}"  "lmbench treatment-default QPS=$qps"  $TUNABLE_IMG  "0.75" "1"   $qps
done
