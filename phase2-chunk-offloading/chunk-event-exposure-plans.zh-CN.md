...
### Dynamo 侧：方案 A 需要什么改动（代码定位）
Dynamo 的 KV router（`lib/kv-router`）和 llm-d 同构——从 token 算 local hash（xxh3，`protocols.rs`）+ 一个
external→local 反查映射——但**现在两者都是 1:1**，所以方案 A 要改两处。

**现状：**
- **转换层是 1:1**（`zmq_wire/convert.rs`）：`create_stored_blocks` 按 `block_hashes.len()` 驱动——
  `num_block_tokens = vec![block_size; block_hashes.len()]`，循环**每个 external hash 吃 `block_size` 个 token**、
  各算一个 local `tokens_hash`。它**不**像 llm-d 那样按 `len(token_ids)/block_size` 切。
- **external→local 映射是 1:1**（`indexer/concurrent_radix_tree.rs`）：
  `type WorkerLookup = FxHashMap<ExternalSequenceBlockHash, SharedBlock>`——一个 external hash → **一个** node，
  不是 `Vec`。radix tree 的 children 按 `tokens_hash`（local、prefix-chained）建；`WorkerLookup` 是 external→node
  的反查，用于 O(1) removal（`apply_removed` 每个 external hash 删一个 node）。这就是你记的那个映射——**确认 1:1，
  不是 list**。

**方案 A 需要的改动（对标 llm-d 的 1:many）：**
1. **转换层 1:1 → 1:many**（`create_stored_blocks`）：当事件是「1 个 tail hash + N×block_size 的 token、
   `block_size`=单块大小」时，循环改由 **token 长度**驱动（`len(token_ids) / kv_block_size`），把整段 token 切成 N 个
   canonical block、逐块算 prefix-chained 的 `tokens_hash`，并把这 N 个**都关联到那一个 external tail hash**。
   （现状只取前 `block_size` 个 token → 1 个 block，还把 tail hash 错配给第 0 块的 local hash，其余 token 丢弃。）
2. **`WorkerLookup` 1:1 → 1:many**：`FxHashMap<ExternalSequenceBlockHash, SharedBlock>` → 改成
   `… Vec<SharedBlock>>`（或等价），让单个 tail 的 `BlockRemoved` 能连带删掉全部 N 个 node。**这正是「local 侧是不是
   list」的答案——现在不是；方案 A 需要它变成 list。**

**两个坑：**
- `create_stored_blocks` 在 `num_tokens != kv_block_size` 时会 break/跳过（`convert.rs`）。方案 A 的 `block_size`=单块
  大小=`kv_block_size`，不冲突；但这段逻辑现在表示"每个 hash 发一个 block"，所以核心改动是改成按 token 长度切。
- `apply_stored` 有告警 `"block_hash mismatch: sequence hashes should be uniform across workers"`
  （`concurrent_radix_tree.rs`）：同一个 `tokens_hash` 期望对应一致的 external hash。chunk 1:many 下，同一段前缀若在
  一条路径上以 by-block（各块各自 hash）、另一条以 chunk（都用 tail hash）出现，可能触发，需评估是否放宽。

**llm-d ↔ Dynamo 改动对照：** llm-d 在 `index.Add` 用 `len(eng):len(req)` 比例推断 1:many + 一对多的
`engineToRequestKeys`；Dynamo 要做等价的——在 `create_stored_blocks` 按 token 长度切 + 把 `WorkerLookup` 改成一对多。

### Dynamo 侧：已实现（落地内容）
分支 `feat/kv-router-chunked-offload-1many`（已 push 到 fork）。1→N 展开一部分在转换层、一部分在索引里，和 llm-d 同构。

**1. 转换层 fan-out**（`zmq_wire/convert.rs`）。`create_stored_blocks` 现在会识别 chunked 事件
（`num_canonical = token_ids.len() / kv_block_size > block_hashes.len()`），调用新的 `create_stored_blocks_fanout`：
把整段 token 切成 `kv_block_size` 的块，逐块算 local `tokens_hash`，并把**这些块全部映射到那一个 chunk-tail 外部 hash**
（比例 `block_hashes[i * m / num_canonical]`，和 llm-d 一致）。1:1 路径不变（普通事件一块一个 hash ⇒
`num_canonical == block_hashes.len()` ⇒ 不进这个分支）。

**2. external→node 反查在三棵树里都改成 1:many：**
- `radix_tree.rs`（单线程）和 `concurrent_radix_tree.rs`：`Ext → node` → `Ext → Vec<node>`。store 时 append；
  parent 解析取 `.last()`（chunk 尾块）；单个 tail 的 `BlockRemoved` 把该 hash 下**所有**节点删掉。
- `concurrent_radix_tree_compressed`：`edge_index` 本来就保留每个 hash 的**最后**位置（lookup/parent 正好要这个），
  所以只改了 `remove_worker_for_hash`——删除时截断到该 hash 的**第一个**边内位置，从而把整个 chunk 取消覆盖。

**3. 对 chunk 安全的自引用检查。** 旧的「按外部 hash 复用节点」技巧（顺带能检测环）和 chunk 冲突（多个块共享一个 hash），
所以删掉，换成显式检查：仅当某个块的 hash == 事件的 `parent_hash`（真正成环）时才拒绝。chunk 的块不会触发（它们的 tail
hash ≠ 上一个 chunk 的 tail）。

**4. 不支持情况的安全守卫**（chunked offload 只支持纯文本 / full-attention）：
- **非 full-attention**（sliding-window / SSM / mamba）：在 `convert_event` 之前就被 normalizer/filter（`is_main_attention`）
  drop 了——不用改。
- **多模态（mm）**：fan-out 无法把 per-sub-block 的 extra_keys 分配下去，所以带 mm（`block_mm_infos`）的 chunked 事件在
  `create_stored_blocks` 里 **drop + warn**。1:1 路径仍然正确处理 mm。
- **`cache_salt` / `prompt_embeds`**：router 看不到（反序列化时丢了），Dynamo 这边没法守——只能 vLLM 侧处理。

**生产路径：** `convert_event`（`zmq_wire`）→ `emit()`（`publisher/sinks.rs`）→ `indexer.apply_event_with_buffer`
→ 后端树 `apply_event`（→ `apply_stored` / `apply_removed`）。

**验证：** 跨实现的 Plan-A chunk 测试（`indexer/tests.rs`，single / concurrent / compressed 都跑：1 个 tail hash →
全部 `factor` 个块可匹配 → 单 tail remove 全删）+ 一个跑真实转换+树的集成测试（`tests/chunk_plan_a.rs`）；全套
**479 lib + 2 集成测试全绿**。在带驱逐的真实 vLLM 多轮抓包上端到端：328 个 CPU remove 把覆盖率从 94.3% → 5.1%、连续从
68 → 16（剩下的 16 = 一直热的共享前缀），证明 chunk 的 store **和** remove 都被正确应用。