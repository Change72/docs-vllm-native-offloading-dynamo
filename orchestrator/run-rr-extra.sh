#!/usr/bin/env bash
# Extra rr sweep: fill in 0.25, 0.75, 2.0, 4.0 for both baseline + best
set +e; set +u
...
log() { echo "[$(date '+%H:%M:%S')] $*" | tee -a $RESULTS/_rrextra.log; }
...
cold_restart() {
  ...
  kubectl delete dgd $DGD -n $NS --ignore-not-found 2>&1 | head -1 >> $RESULTS/_rrextra.log
  sleep 15
  gen_dgd_yaml "$image" "$h" "$disk" "$pl" "$cpu_bytes" | kubectl apply -f - 2>&1 | head -1 >> $RESULTS/_rrextra.log
  wait_ready
}
...
run_cycle() {
  ...
  log ""
  log "============================================================"
  log "RREXTRA $cycle_id  | $label | rr=$rr"
  log "============================================================"
  ...
  collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "$workload_json" "$c" "$rr" "OK_RREXTRA"
}

log "=========================="
log "RR EXTRA SWEEP START $(date)"
log "=========================="

LONG=generate_multi_turn_longbench.json

# 4 new rr values × 2 images. Order: easy/fast (lower rr) → hard (higher rr)
# Note: rr 越高 client 越快 → bench 越快完，但 server 早就饱和
for rr in 0.25 0.75 2.0 4.0; do
  run_cycle "t6-baseline-rr${rr}_EXTRA"  "baseline rr=$rr"  $BASELINE_IMG ""    ""     ""    $LONG 64 $rr
  run_cycle "t6-best-rr${rr}_EXTRA"      "best rr=$rr"      $TUNABLE_IMG  "1.0" "0.25" "100" $LONG 64 $rr
done

log ""
log "=========================="
log "RR EXTRA COMPLETE $(date)"
log "=========================="
sleep infinity
