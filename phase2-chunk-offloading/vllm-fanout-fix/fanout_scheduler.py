...
class _OffloadEventMetadata:
    """Full BlockStored payload snapshot for one OffloadKey.

    Captured while Request context is available and drained when the
    OffloadingManager later emits the matching event. ``medium`` is not
    stored; it is forwarded from the OffloadingEvent.
    """

    # Currently length 1: one offload key maps to one hash block.
...
class OffloadingConnectorScheduler:
    ...
    def __init__(self, spec: OffloadingSpec):
        ...
        # OffloadKey -> BlockStored payload. Only populated for the supported
        # 1:1 full-attention shape; other shapes fall back to placeholders.
    ...
    def take_events(self) -> Iterable[KVCacheEvent]:
        ...
        Complete metadata is available only for the supported 1:1
        full-attention shape. Other shapes retain the previous placeholder
    ...
    def _take_stored_event(self, event: OffloadingEvent) -> Iterable[KVCacheEvent]:
        for key in event.keys:
            ...
            if meta is None:
                # Unsupported shapes intentionally keep the old placeholder
                # payload. Warn once so expected fallbacks do not flood logs.
                logger.warning_once(
                    "OffloadingConnectorScheduler: no event metadata for "
                    "offload key during BlockStored emission; emitting a "
                    "placeholder payload. Expected for block_size_factor "
                    "> 1, hash_block_size_factor > 1, or non-full-"
                    "attention groups; otherwise indicates a missing "
                    "populate path."
                )
                yield BlockStored(
                    block_hashes=[
    ...
    def reset_cache(self) -> None:
        ...
        # The manager pool is empty; pending event payloads are stale.
