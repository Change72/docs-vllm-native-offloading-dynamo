#!/usr/bin/env bash
# Phase 5: Defensive experiments (~5-6h)
#  Block 2: LMBench rerun with polling bug fixed + RPS collection (12 cycles, ~1h)
#  Block 3: c=128 statistical repeats × 4 cohorts × 3 reps (12 cycles, ~1.5h)
#  Block 4: pre-warm ablation (4 cycles, ~1.5h)
#  Block 5: best-image ceiling at c=192/256/384 (3 cycles, ~0.75h)
set +e; set +u
...
def parse_bench():
    ...
    # LMBench fallback: parse "finished one request" lines
    if 'rps' not in m or m['rps'] == 0:
        finished = len(re.findall(r'finished one request', t))
        # Try to detect duration from log: TEST_DURATION env was 200s, but use elapsed from log timestamps
        timestamps = re.findall(r'\[(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})', t)
        if len(timestamps) > 10 and finished > 0:
...
run_multi_turn() {
  ...
  collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "$workload" "$c" "$rr" "OK_PH5"
}

# Run benchmark WITHOUT cold-restart (i.e., reuse the current DGD - for prewarm second run)
run_multi_turn_nocold() {
  local cycle_id=$1; local label=$2; local image=$3
  local h=$4; local disk=$5; local pl=$6
  local workload=$7; local c=$8; local rr=$9
  log ""
  log "============================================================"
  log "PHASE5 $cycle_id  | $label  | c=$c rr=$rr  [no cold-restart, warm cache]"
  log "============================================================"
  log "  launching bench on existing DGD..."
  kubectl exec $CLIENT -n $NS -- bash -c "
    cd /work/multi_turn
    rm -f /work/_bench.log /work/_stats.json
    nohup python3 benchmark_serving_multi_turn.py \\
  ...
  dump_artifacts "$cycle_id"
  collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "$workload" "$c" "$rr" "OK_PH5_WARM"
}

run_lmbench_v2() {
  # Fixed version: wait for >=2 "Performance summary" AND elapsed > 220s
  local cycle_id=$1; local label=$2; local image=$3; local h=$4; local pl=$5; local qps=$6
  local disk=""; [ -n "$h" ] && disk="0.25"
  log ""
  log "============================================================"
  log "PHASE5-LMBENCH $cycle_id  | $label  QPS=$qps"
  log "============================================================"
  cold_restart "$image" "$h" "$disk" "$pl"
  [ $? -ne 0 ] && { log "[FAIL]"; collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "lmbench" "64" "$qps" "FAIL_NOREADY"; return; }
  log "  launching LMBench QPS=$qps (will wait full 220s+)..."
  kubectl exec $CLIENT -n $NS -- bash -c "
    cd /work/lmbench
    rm -f /work/_lmbench.log /work/_lmbench_output_${qps}.csv
    ...
    nohup ./long_input_short_output_run.sh 'Qwen/Qwen3-32B' \\
      ...
      '/work/_lmbench' $qps > /work/_lmbench.log 2>&1 &
  " >> $LOG
  # Minimum 4 min wait (warmup 100s + run 200s + buffer), then check
  sleep 240
  for i in $(seq 1 30); do
    sleep 20
    local done=$(kubectl exec $CLIENT -n $NS -- bash -c "grep -c 'Performance summary' /work/_lmbench.log 2>/dev/null" 2>/dev/null | tr -d '\n' || echo 0)
    [ "$done" -ge 2 ] && { log "  [done at $((4 + i/3))min, summaries=$done]"; sleep 5; break; }
    [ $((i % 3)) = 0 ] && log "  poll $((4 + i/3))min summaries=$done"
  done
  local dir=$RESULTS/$cycle_id; mkdir -p $dir
  kubectl cp "$NS/$CLIENT:/work/_lmbench.log" "$dir/bench.log" 2>/dev/null
  kubectl cp "$NS/$CLIENT:/work/_lmbench_output_${qps}.csv" "$dir/lmbench.csv" 2>/dev/null
  for w in $(kubectl get pod -n $NS -o name 2>/dev/null | grep vllmdecodeworker | cut -d/ -f2); do
    short=$(echo $w | rev | cut -d- -f1 | rev)
    kubectl exec "$w" -n $NS -- curl -sf http://localhost:9090/metrics > "$dir/metrics_worker_${short}.txt" 2>/dev/null &
  done
  wait
  collect_metrics "$cycle_id" "$label" "$image" "$h" "$disk" "$pl" "lmbench" "64" "$qps" "OK_PH5_LM"
}

log "=========================="
log "PHASE 5 START $(date)"
log "=========================="

LONG=generate_multi_turn_longbench.json

# ---------- Block 2: LMBench rerun w/ polling bug fixed — 12 cycles ----------
# Only re-run the 2 cohorts that had truncated runs in Phase 4 (baseline+pl=100 and treatment-default).
# Phase 3 baseline-default and best already have valid full-duration runs.
log ""; log "##### Block 2: LMBench rerun fix (baseline+pl=100, treatment-default with valid RPS) #####"
for qps in 1 4 8 16 32 64; do
  run_lmbench_v2 "ph5-lm-baseline-pl100-qps${qps}"    "lmbench baseline+pl100 QPS=$qps"    $BASELINE_IMG ""    "100" $qps
  run_lmbench_v2 "ph5-lm-treatment-default-qps${qps}" "lmbench treatment-default QPS=$qps" $TUNABLE_IMG  "0.75" "1"   $qps
done

# ---------- Block 3: Statistical repeats on c=128 — 12 cycles ----------
log ""; log "##### Block 3: c=128 × 4 cohorts × 3 repeats (error bars) #####"
for rep in 1 2 3; do
  run_multi_turn "ph5-rep${rep}-baseline-default-c128"  "rep${rep} baseline-default c=128"  $BASELINE_IMG ""    ""    ""    $LONG 128 0.5
  run_multi_turn "ph5-rep${rep}-baseline-pl100-c128"    "rep${rep} baseline+pl100 c=128"    $BASELINE_IMG ""    ""    "100" $LONG 128 0.5
  run_multi_turn "ph5-rep${rep}-treatment-default-c128" "rep${rep} treatment-default c=128" $TUNABLE_IMG  "0.75" "0.25" "1"  $LONG 128 0.5
  run_multi_turn "ph5-rep${rep}-best-c128"              "rep${rep} best c=128"              $TUNABLE_IMG  "1.0" "0.25" "100" $LONG 128 0.5
done

# ---------- Block 4: Pre-warm ablation — 6 cycles (2 cohorts × 3 runs each) ----------
log ""; log "##### Block 4: Pre-warm ablation (cold-start vs warmed) #####"
# For each cohort: (a) cold + measure c=64, (b) cold + warmup c=8, (c) WARM c=64 (no restart).
# Compare (a) vs (c): if (c) has lower compute %, warmup helped → 'head start' theory holds.
# If (a) ≈ (c), the gap is mechanism-driven (routing intelligence), not initial bias.
do_prewarm_pair() {
  local name=$1; local img=$2; local h=$3; local disk=$4; local pl=$5
  run_multi_turn "ph5-prewarm-${name}-cold-c64"   "prewarm ${name} cold c=64"   "$img" "$h" "$disk" "$pl" $LONG 64 0.5
  log "  [prewarm phase 1 for ${name}: c=8 fills cache, no cold restart after]"
  run_multi_turn "ph5-prewarm-${name}-warmup-c8"  "prewarm ${name} warmup c=8"  "$img" "$h" "$disk" "$pl" $LONG 8 1.0
  log "  [prewarm phase 2 for ${name}: c=64 on warmed cache, NO restart]"
  run_multi_turn_nocold "ph5-prewarm-${name}-warm-c64" "prewarm ${name} WARM c=64" "$img" "$h" "$disk" "$pl" $LONG 64 0.5
}
do_prewarm_pair "baseline-default" "$BASELINE_IMG" ""    ""     ""
do_prewarm_pair "best"             "$TUNABLE_IMG"  "1.0" "0.25" "100"

# ---------- Block 5: Best image ceiling — 3 cycles ----------
log ""; log "##### Block 5: Best-image scaling ceiling #####"
# Need num_conv >= c. At conv_prefix=15K and corpus=2.6M, max conversation count is ~170.
# So we cap concurrency at c=128 already. For higher c, use saturate workload (small prefix, many conv).
SAT=generate_multi_turn_saturated.json
for c in 192 256 384; do
  run_multi_turn "ph5-ceiling-best-c${c}-saturate" "ceiling best c=$c (saturate)" $TUNABLE_IMG "1.0" "0.25" "100" $SAT $c 0.5
done

log ""
log "=========================="
log "PHASE 5 COMPLETE $(date)"
log "=========================="
sleep infinity
